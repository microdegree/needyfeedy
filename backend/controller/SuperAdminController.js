...
router.post("/addUser", (request, response) => {
    ...
    database.collection("needyfeedyUserTable").insertOne(request.body, (error, result) => {
        if (error) {
...
router.get("/getAllUsers", (request, response) => {
    ...
    database.collection("needyfeedyUserTable").find({}).toArray((error, result) => {
        if (error) {
            return response.status(500).send(error);
        }
...
router.post("/modifyUser", (request, response) => {
    ...
    database.collection("needyfeedyUserTable").replaceOne({ _id: objectId }, request.body, (error, result) => {
        if (error) {
            console.log(error)
...
router.post("/deleteUser", (request, response) => {
    ...
    var objectId = new ObjectId(request.body._id);
    delete request.body._id
    database.collection("needyfeedyUserTable").deleteOne({ _id: objectId }, (error, result) => {
        if (error) {
            console.log(error)
