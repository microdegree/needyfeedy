...
router.post("/addProject", (request, response) => {
    ...
    database.collection("donorInfoTable").update({ "email": request.body.email },
        request.body,
        { upsert: true },
        (error, result) => {
...
router.post("/getMyProjects", (request, response) => {
    ...
    database.collection("donorInfoTable").find(request.body).toArray((error, result) => {
        if (error) {
            console.log(error)
...
router.post("/modifyProject", (request, response) => {
    ...
    database.collection("donorInfoTable").replaceOne({ 'email': request.body.email }, request.body, (error, result) => {
        if (error) {
            console.log(error)
...
router.post("/getMyBookings", (request, response) => {
    ...
    console.log('In getMyProjects', request.body)
    database.collection("mealsRequestTable").find(request.body).toArray((error, result) => {
        if (error) {
            console.log(error)
            return response.status(500).send(error);
        }
...
router.post('/uploadDonorImage', upload.single('avatar'), function (request, response) {
